package com.laputa.controller.system.head;
...
import com.laputa.service.system.fhsms.FhsmsManager;
import com.laputa.service.system.user.UserManager;
import com.laputa.service.system.userphoto.UserPhotoManager;
...
import com.laputa.util.SmsUtil;
import com.laputa.util.Tools;
import com.laputa.util.Watermark;
import com.laputa.util.mail.SimpleMailSender;
...
//						userList = "appuser".equals(fmsg) ? appuserService.listAllUser(pd):userService.listAllUser(pd);
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",laputa,"+pd.getString("PORT")+",laputa,"+pd.getString("EMAIL")+",laputa,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",laputa,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",laputa,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",laputa,"+pd.getString("fcontent")+",laputa,"+pd.getString("fontSize")+",laputa,"+pd.getString("fontX")+",laputa,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",laputa,"+pd.getString("imgUrl")+",laputa,"+pd.getString("imgX")+",laputa,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
