package com.laputa.controller.system.login;
...
import com.laputa.service.business.sensor.SensorManager;
...
import com.laputa.service.system.buttonrights.ButtonrightsManager;
import com.laputa.service.system.fhbutton.FhbuttonManager;
import com.laputa.service.system.fhlog.FHlogManager;
import com.laputa.service.system.loginimg.LogInImgManager;
import com.laputa.service.system.menu.MenuManager;
import com.laputa.entity.system.Menu;
import com.laputa.entity.system.Role;
import com.laputa.entity.system.User;
import com.laputa.service.system.role.RoleManager;
import com.laputa.service.system.user.UserManager;
import com.laputa.util.AppUtil;
import com.laputa.util.Const;
import com.laputa.util.DateUtil;
import com.laputa.util.Jurisdiction;
import com.laputa.util.PageData;
import com.laputa.util.RightsHelper;
...
//	private AppuserManager appuserService;
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("ep1234pe", "").replaceAll("ep1234pe", "").split(",ep,");
		if(null != KEYDATA && KEYDATA.length == 3){
			Session session = Jurisdiction.getSession();
			String sessionCode = (String)session.getAttribute(Const.SESSION_SECURITY_CODE);		//获取session中的验证码
			String code = KEYDATA[2];
//			if(null == code || "".equals(code)){//判断效验码		//quchuyanzhengma sommer 20170425
//				errInfo = "nullcode"; 			//效验码为空			//quchuyanzhengma sommer 20170425
//			}else{		//quchuyanzhengma sommer 20170425
				String USERNAME = KEYDATA[0];	//登录过来的用户名
				String PASSWORD  = KEYDATA[1];	//登录过来的密码
				pd.put("USERNAME", USERNAME);
//				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码 		//quchuyanzhengma sommer 20170425
...
//				}else{												//quchuyanzhengma sommer 20170425
//					errInfo = "codeerror";				 	//验证码输入有误 		//quchuyanzhengma sommer 20170425
//				}		//quchuyanzhengma sommer 20170425
...
//		pd.put("appUserCount", Integer.parseInt(appuserService.getAppUserCount("").get("appUserCount").toString()));	//会员数
		pd.put("sensor",sensorService.listAll(null).size() +"");
//		pd.put("parking",Integer.parseInt(parkingService.getParkingCount("").get("parkingCount").toString()));
		List<PageData>	parkerList = parkingService.listAll(null);	//列出Battery列表
		pd.put("longitude",120.3054559);
		pd.put("latitude",31.57003745);
		mv.addObject("parkerList", parkerList);
//		pd.put("sensor",sensorService.listAll(null).size() +"");
		mv.addObject("pd",pd);
